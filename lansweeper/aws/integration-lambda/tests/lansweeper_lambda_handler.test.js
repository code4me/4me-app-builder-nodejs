...
it('handles refresh GET call with labelGenerator lansweeper_asset_name', async () => {
  const handlerResult = await checkRefreshEvent(refreshGETEvent,
                                                'test-account-get',
...
it('handles refresh SQS with labelGenerator lansweeper_asset_name', async () => {
  const handlerResult = await checkRefreshEvent(refreshSQSEvent,
                                                'test-account-sqs',
                                                {importType: "no_ip_only", labelGenerator: 'lansweeper_asset_name'},
                                                false,
                                                true);
...
it('handles refresh SQS with labelGenerator autogenerated', async () => {
  const handlerResult = await checkRefreshEvent(refreshSQSEvent,
                                                'test-account-sqs',
                                                {importType: "no_ip_only", labelGenerator: 'autogenerated'},
                                                false,
                                                false);
...
describe('summary formatting', () => {
  it('handles basic summaries', () => {
    const handler = new LansweeperLambdaHandler({});
    expect(handler.formatSummary({a: "123", b: 1})).toEqual("```\n{\n  \"a\": \"123\",\n  \"b\": 1\n}\n```\n");
  });

  it('handles too long summaries', () => {
    const handler = new LansweeperLambdaHandler({});
    ...
    const maxSupported = largeValue.substring(0, LansweeperLambdaHandler.MAX_SUMMARY_SIZE - 10);
    expect(summary.substring(54, summary.length - 5)).toEqual(maxSupported);
  });
...
async function checkRefreshEvent(event, expectedCustomerAccount, extraConfig, expectedNetworkedAssetsOnly, expectedGenerateLabels = false) {
  const providerAccessToken = {access_token: 'howard.tanner'};
  const customerAccessToken = {access_token: 'foo.bar'};
  const customerSecrets = {
    application: {
