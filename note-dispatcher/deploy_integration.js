...
class DeployIntegration {
  ...
  async createOffering(serviceInstance, lambdaUrl) {
    const offeringInput = this.getOfferingInput();
    offeringInput.serviceInstanceId = serviceInstance.id;
    offeringInput.webhookUriTemplate = `${lambdaUrl}?account={account}`;
...
(async () => {
  ...
  const offering = await deployIntegration.createOffering(serviceInstance, lambdaUrl);
